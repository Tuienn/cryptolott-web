...
import { TicketLine, NumberGrid, PriceFooter } from "@/components/lottery"
...
export function NumberSelectPage({ slug }: NumberSelectPageProps) {
  ...
  // Get the line being edited from URL params
  const editingLineId = searchParams.get("line")

  const [lines, setLines] = React.useState<NumberLine[]>(getInitialLines)
  const [selectedNumbers, setSelectedNumbers] = React.useState<number[]>([])
  const [activeLineId, setActiveLineId] = React.useState<string | null>(editingLineId)
  ...
  return (
    <div className="flex flex-col pb-24">
      ...
      <div className="border-b border-dashed border-border bg-muted/30">
